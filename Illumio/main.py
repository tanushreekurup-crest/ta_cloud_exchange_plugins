...
class IllumioPlugin(PluginBase):
    ...
    def get_modified_indicators(self, indicators):
        ...
        available_workloads = []
        if int(self.configuration.get('retraction_interval', 0)) == 0:
            self.logger.info(f"{self.log_prefix}: Retraction Interval set to zero, Skipping fetchinng of Retracted IoC(s).")
            yield [], True
        try:
            ...
            if not workloads:
                self.logger.info(
                    f"{self.log_prefix}: No Workloads found containing the Label Scope(s) - "
                    f"'{self.configuration.get('label_scope')}'. All indicators needs to be retracted."
                )
            else:
                self.logger.debug(
                ...
                for workload in workloads:
                    ...
                    self.logger.info(
                        ...
                        f"Total indicator(s) fetched till now for IoC(s) retraction."
                    )
                info_msg = (
                    f"Successfully fetched {len(available_workloads)} indicator(s)."
                )
                self.logger.info(
        ...
        retracted_indicators = list(stored_indicators - set(available_workloads))
